...
$result = SQL::select(Media::TBNAME, array('active' => 1, 'artist'=>array(ARTISTID_MAIN,ARTISTID_SECOND,ARTISTID_THIRD)), '*', 'id ASC');
...
echo '<table id="full_list_media" class="mainlist">';
    echo '<tr>';
        ...
        echo '<th class="w10">'.$lang->l('disc_id').'</th>';
        echo '<th></th>';
    echo '</tr>';
    while($item = $result->fetch_assoc()){
        echo '<tr class="">';
            ...
            echo '<td class="w5">'.$item['lancement'].'</td>';
            echo '<td class="w5">'.$item['vues'].'</td>';
            echo '<td class="w10">'.$item['duree'].'</td>';
            echo '<td class="w10">'.(($item['type']=='1') ? 'Vidéo' : 'Audio').'</td>';
            echo '<td class="w10">'.$item['albumid'].'</td>';
            echo '<td>';
